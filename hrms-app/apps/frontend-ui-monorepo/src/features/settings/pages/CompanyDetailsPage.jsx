  ...
  const Field = ({ label, field, type = 'text' }) => (
    <Box>
      ...
      {editing ? (
        ...
        <Text fontSize="sm" color="text-secondary" py={2} px={3} bg="app-bg-secondary" borderRadius="md" minH="38px">
          {form[field] || <Box as="span" color="text-muted">—</Box>}
        </Text>
      )}
    </Box>
  );
    ...
    <DashboardLayout>
      <Box px={{ base: 4, md: 8 }} py={6} maxW="4xl">
        ...
        <Flex justify="space-between" align="center" mb={6}>
          <Box>
            <Text fontSize="xl" fontWeight="bold" color="text-primary">Company Details</Text>
            <Text fontSize="sm" color="text-muted">Change / Update Company details</Text>
          </Box>
          {!editing && (
        ...
        <Box bg="card-bg" borderRadius="xl" boxShadow="sm" border="1px solid" borderColor="border-color" p={8}>
          <VStack spacing={8} align="stretch">
            ...
            {/* Basic Info */}
            <Box>
              <Text fontWeight="semibold" fontSize="sm" color="text-secondary" mb={4} textTransform="uppercase" letterSpacing="wide">
                Basic Information
              </Text>
